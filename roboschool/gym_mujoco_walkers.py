...
from roboschool.gym_forward_walker import RoboschoolForwardWalker, JvonForwardWalker
...
class JvonForwardWalkerMujocoXML(JvonForwardWalker, RoboschoolMujocoXmlEnv):
    def __init__(self, fn, robot_name, action_dim, obs_dim, power):
        RoboschoolMujocoXmlEnv.__init__(self, fn, robot_name, action_dim, obs_dim)
        JvonForwardWalker.__init__(self, power)
...
class JvonAnt(JvonForwardWalkerMujocoXML):
    foot_list = ['front_left_foot', 'front_right_foot', 'left_back_foot', 'right_back_foot']
    def __init__(self):
        RoboschoolForwardWalkerMujocoXML.__init__(self, "jvon_ant.xml", "torso", action_dim=8, obs_dim=1, power=2.5)
    def alive_bonus(self, z, pitch):
        return +1 if z > 0.26 else -1  # 0.25 is central sphere rad, die if it scrapes the ground
